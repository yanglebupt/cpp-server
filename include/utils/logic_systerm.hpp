  ...
  void HandleMessages()
  {
    ...
    while (!_exit)
    {
      ...
      if (_exit) // 即将退出，取出剩余的消息进行处理，再结束
      {
        ...
        {
          owned_message_interface<T, Connection> msg = message_in_dq.pop_front();
...
public:
  virtual void OnMessage(std::shared_ptr<Connection> client, T &msg) = 0;
  ...
  tsqueue<owned_message_interface<T, Connection>> &InComing()
  {
