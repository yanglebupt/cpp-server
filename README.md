...
### connection 避免客户端断开导致连接的二次释放？

**Q:** 如果在读之后还注册了并仍然存在写回调，那么读回调失败释放之后，写回调也会失败，此时在写回调中可能出现空指针异常（因为读失败已经释放了资源），该咋办呢？

**A:** 每次注册回调都往回调函数里面传入智能指针 `shared_from_this()`，引用加一，那么读回调失败中，移除连接对象引用不会为 0，只会减一，当所有注册的回调函数执行完毕后，引用才会为 0，此时在自动触发析构，`socket.close` 关闭读写通道，释放套接字资源

**A:** 什么时候引用才会为 0

- 读任务始终存在，除非读失败（失败情况多数是 EOF，对端关闭），读回调里面不再继续注册读任务
- 发送队列清空，并且用户没有调用 send 继续发送消息

### 如何优雅的关闭连接？

1. 主动方从 `connected` 状态切换到 `halfclose` 状态，此时不再允许 send 发送新的消息了，等待发送队列清空后，调用 `socket.shutdown_send` 发送 FIN 包，触发第一次挥手

2. 被动方接收到后（也就是读失败的 EOF），直接释放主 connection（计数减一），等待其他回调任务完成，触发析构，`socket.close` 关闭读写通道，释放套接字资源，发送 FIN 包，触发第三次挥手

3. 主动方接收到后（也就是读失败的 EOF），再释放主 connection（计数减一），等待其他回调任务完成，触发析构，`socket.close` 关闭读写通道，释放套接字资源。

至此四次挥手完成，第二/第四次挥手是 ACK，不需要手动控制，会自动回。
...
- 通过监听退出信号或者主动调用，执行服务器退出逻辑

  1. 不再接收新的请求了，也就是停止 `async_accept` 任务
  2. 通知消息线程不用堵塞了，等待剩余消息处理完毕，消息线程结束
  2. 主动关闭全部客户端的连接，如上一个问题所示，并停止线程连接池
  3. 此时主线程也不再堵塞，向下执行结束了，释放 `server` 类，触发其析构函数，析构函数里面回收线程资源
...
[`SIGINT, SIGTERM, SIGBREAK`](https://blog.csdn.net/weixin_60461563/article/details/132721374) 目前监听了这些信号，在 windows 上还可以通过 [SetConsoleCtrlHandler](https://asawicki.info/news_1465_handling_ctrlc_in_windows_console_application) 来处理退出
